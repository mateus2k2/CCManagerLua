...
__a.__index=__a;bd=setmetatable(__a,bd)end;return bd end end end;return bc end}end;aa["objects"]={}
...
db=os.startTimer(_d)end elseif(ad==-1)then db=os.startTimer(_d)end end end}ac.__index=ac;return setmetatable(ac,cb)end end;aa["libraries"]={}
aa["libraries"]["basaltMon"]=function(...)
local ab,bb=math.max,string.sub
return
function(cb)local db={}local _c,ac=0,0;local bc,cc=1,1;local dc,_d=colors.black,colors.white
for __a,a_a in pairs(cb)do
db[__a]={}for b_a,c_a in pairs(a_a)do
...
ac:getHeight())end;bc:show()end}end}end;return aa["main"]()