...
aa["objects"]["Radio"]=function(...)local ab=da("utils")
local bb=da("tHex")
return
function(cb,db)local _c=db.getObject("List")(cb,db)
_c:setType("Radio")_c:setSize(1,1)_c:setZ(5)
...
aba+caa-1,_c(daa,cb(1 -baa+1,1),cb(bba-baa+1,1)))end end end;c_a.__index=c_a;return setmetatable(c_a,dc)end end;aa["libraries"]={}
...
local cb=true;local db={top=false,bottom=false,left=false,right=false}
...
dcb.timerId=os.startTimer(0.05)else dcb.finished()acb:doneHandler(ccb)end end end end}return _cb end}end;return aa["main"]()